...
Begin VB.Form ABMCanal 
   BorderStyle     =   1  'Fixed Single
   Caption         =   " ABM de Canales"
   ClientHeight    =   4245
   ClientLeft      =   4065
   ClientTop       =   3105
   ClientWidth     =   5895
   ...
   Begin VB.CommandButton CmdBorrar 
      Caption         =   "&Eliminar"
      DisabledPicture =   "ABMCanal.frx":030A
      Enabled         =   0   'False
      Height          =   720
      Left            =   4095
      Picture         =   "ABMCanal.frx":0614
      Style           =   1  'Graphical
      TabIndex        =   4
      Top             =   3480
      Width           =   840
   ...
   Begin VB.CommandButton CmdGrabar 
      Caption         =   "&Guardar"
      DisabledPicture =   "ABMCanal.frx":0C28
      Enabled         =   0   'False
      Height          =   720
      Left            =   2385
      Picture         =   "ABMCanal.frx":0F32
      Style           =   1  'Graphical
      TabIndex        =   2
      Top             =   3480
      Width           =   840
   ...
   Begin TabDlg.SSTab TabTB 
      ...
      EndProperty
      TabCaption(0)   =   "&Datos"
      TabPicture(0)   =   "ABMCanal.frx":123C
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).Control(0)=   "Frame3"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "&Buscar"
      TabPicture(1)   =   "ABMCanal.frx":1258
...
Private Sub CmdBorrar_Click()
    ...
    If Trim(TxtCodigo) <> "" Then
        resp = MsgBox("Seguro desea eliminar el Canal: " & Trim(TxtDescrip) & "? ", 36, "Eliminar:")
        If resp <> 6 Then Exit Sub
        
        Screen.MousePointer = 11
        lblEstado.Caption = "Eliminando ..."
        
        DBConn.Execute "DELETE FROM CANALES WHERE CAN_CODIGO = " & XN(TxtCodigo)
        lblEstado.Caption = ""
...
Private Sub CmdBuscAprox_Click()
    ...
    Screen.MousePointer = 11
    Me.Refresh
    sql = "SELECT CAN_CODIGO,CAN_DESCRI FROM CANALES "
    If Trim(TxtDescriB) <> "" Then sql = sql & "WHERE CAN_DESCRI LIKE '%" & Trim(TxtDescriB) & "%'"
    sql = sql & " ORDER BY CAN_DESCRI"
...
Private Sub cmdGrabar_Click()
    ...
    If TxtCodigo.Text <> "" Then
        sql = "UPDATE CANALES "
        sql = sql & " SET CAN_DESCRI = " & XS(TxtDescrip)
        sql = sql & " WHERE CAN_CODIGO = " & XN(TxtCodigo)
        DBConn.Execute sql
        
    Else
        TxtCodigo = "1"
        sql = "SELECT MAX(CAN_CODIGO) as maximo FROM CANALES"
        rec.Open sql, DBConn, adOpenStatic, adLockOptimistic
        If Not IsNull(rec.Fields!Maximo) Then TxtCodigo = XN(rec.Fields!Maximo) + 1
        rec.Close
        
        sql = "INSERT INTO CANALES(CAN_CODIGO,CAN_DESCRI) "
        sql = sql & " VALUES ("
...
Private Sub TxtCodigo_LostFocus()
    If TxtCodigo.Text <> "" Then
        ...
        rec.Open sql, DBConn, adOpenStatic, adLockOptimistic
        If rec.EOF = False Then
         TxtDescrip.Text = rec!CAN_DESCRI
        Else
