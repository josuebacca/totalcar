...
Begin VB.Form frmNotaCreditoProveedores 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Nota de Crédito de Proveedores..."
   ClientHeight    =   6675
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8775
   ...
   Begin TabDlg.SSTab tabDatos 
      ...
      EndProperty
      TabCaption(0)   =   "&Datos"
      TabPicture(0)   =   "frmNotaCreditoProveedores.frx":0000
      ...
      TabCaption(1)   =   "&Buscar"
      TabPicture(1)   =   "frmNotaCreditoProveedores.frx":001C
      ...
      Begin VB.Frame Frame4 
         ...
         Begin VB.CommandButton CmdBuscAprox 
            ...
            Left            =   6780
            MaskColor       =   &H00FFFFFF&
            Picture         =   "frmNotaCreditoProveedores.frx":0038
            Style           =   1  'Graphical
            TabIndex        =   23
            ToolTipText     =   "Buscar "
            Top             =   1065
         ...
         Begin VB.CommandButton cmdBuscarProveedor 
            ...
            Left            =   3390
            MaskColor       =   &H000000FF&
            Picture         =   "frmNotaCreditoProveedores.frx":27DA
            Style           =   1  'Graphical
            TabIndex        =   44
      ...
      Begin VB.Frame FrameProveedor 
         ...
         Begin VB.CommandButton cmdBuscarProveedor1 
            ...
            Left            =   2295
            MaskColor       =   &H000000FF&
            Picture         =   "frmNotaCreditoProveedores.frx":2AE4
            Style           =   1  'Graphical
            TabIndex        =   51
            ToolTipText     =   "Buscar Proveedor"
            Top             =   765
   ...
   End
End
Attribute VB_Name = "frmNotaCreditoProveedores"
...
Private Sub CmdBorrar_Click()
    ...
    If MsgBox("¿Seguro que desea eliminar la Nota de Crédito?", vbQuestion + vbYesNo + vbDefaultButton2, TIT_MSGBOX) = vbYes Then
        On Error GoTo Seclavose
         ...
         sql = sql & " AND CPR_NUMERO=" & XN(txtNroComprobante)
         DBConn.Execute sql
...
Private Sub CmdBuscAprox_Click()
     GrdModulos.Rows = 1
    sql = "SELECT CP.*,P.PROV_RAZSOC,TC.TCO_ABREVIA"
    sql = sql & " FROM NOTA_CREDITO_PROVEEDOR CP, TIPO_COMPROBANTE TC, PROVEEDOR P"
    sql = sql & " WHERE"
    sql = sql & " CP.TPR_CODIGO=P.TPR_CODIGO"
    sql = sql & " AND CP.PROV_CODIGO=P.PROV_CODIGO"
    sql = sql & " AND CP.TCO_CODIGO=TC.TCO_CODIGO"
    If (chkTipoProveedor.Value = Checked And chkProveedor.Value = Checked) Or _
       (chkTipoProveedor.Value = Unchecked And chkProveedor.Value = Checked) Then
        ...
        If cboBuscaTipoProveedor.ListIndex <> -1 Then
            sql = sql & " AND CP.TPR_CODIGO=" & cboBuscaTipoProveedor.ItemData(cboBuscaTipoProveedor.ListIndex)
        End If
        If txtProveedor.Text <> "" Then
            sql = sql & " AND CP.PROV_CODIGO=" & XN(txtProveedor)
        End If
        
    ElseIf chkTipoProveedor.Value = Checked And chkProveedor.Value = Unchecked Then
        sql = sql & " AND CP.TPR_CODIGO=" & cboBuscaTipoProveedor.ItemData(cboBuscaTipoProveedor.ListIndex)
    End If
    If Not IsNull(FechaDesde) Then sql = sql & " AND CP.CPR_FECHA>=" & XDQ(FechaDesde)
    If Not IsNull(FechaHasta) Then sql = sql & " AND CP.CPR_FECHA<=" & XDQ(FechaHasta)
    sql = sql & " ORDER BY CP.CPR_FECHA,CP.PROV_CODIGO"
    Rec1.Open sql, DBConn, adOpenStatic, adLockOptimistic
    If Rec1.EOF = False Then
        Do While Rec1.EOF = False
            GrdModulos.AddItem Rec1!PROV_RAZSOC & Chr(9) & Rec1!TCO_ABREVIA & Chr(9) & Format(Rec1!CPR_NROSUC, "0000") & "-" & Format(Rec1!CPR_NUMERO, "00000000") & Chr(9) & _
                               Rec1!CPR_FECHA & Chr(9) & Rec1!TPR_CODIGO & Chr(9) & Rec1!PROV_CODIGO & Chr(9) & _
                               Rec1!CPR_CONCEPTO & Chr(9) & Rec1!TCO_CODIGO & Chr(9) & Rec1!CPR_NROSUC & Chr(9) & _
                               Rec1!CPR_NUMERO & Chr(9) & Rec1!CPR_NETO & Chr(9) & _
                               Rec1!CPR_IVA & Chr(9) & Rec1!CPR_TOTAL & Chr(9) & Rec1!CPR_PERIODO
            Rec1.MoveNext
        Loop
        GrdModulos.SetFocus
    Else
...
Private Sub cmdGrabar_Click()
    ...
    DBConn.BeginTrans
    sql = "SELECT CPR_NETO FROM NOTA_CREDITO_PROVEEDOR"
    sql = sql & " WHERE TPR_CODIGO=" & cboTipoProveedor.ItemData(cboTipoProveedor.ListIndex)
    sql = sql & " AND PROV_CODIGO=" & XN(txtCodProveedor)
    sql = sql & " AND TCO_CODIGO=" & cboComprobante.ItemData(cboComprobante.ListIndex)
    sql = sql & " AND CPR_NROSUC=" & XN(txtNroSucursal)
    ...
    If rec.EOF = False Then
        If MsgBox("La Nota de Crédito ya fue ingresada!!!" & Chr(13) & _
                  "Seguro que modifica la Nota de Crédito", vbQuestion + vbYesNo, TIT_MSGBOX) = vbYes Then
...
'            DBConn.Execute sql
    ...
    Else 'NUEVO GASTO
        ...
        sql = sql & " (TPR_CODIGO,PROV_CODIGO,TCO_CODIGO,CPR_NROSUC,CPR_NUMERO,"
        sql = sql & "CPR_FECHA,CPR_NETO,CPR_IVA,CPR_TOTAL,CPR_SALDO,"
        sql = sql & "CPR_PERIODO,CPR_CONCEPTO,CPR_NROSUCTXT,CPR_NUMEROTXT)"
        sql = sql & " VALUES ("
...
Private Sub CmdSalir_Click()
    Set frmNotaCreditoProveedores = Nothing
    Unload Me
End Sub
