...
Begin VB.Form ABMRubro 
   ...
   Begin VB.CommandButton CmdBorrar 
      Caption         =   "&Eliminar"
      DisabledPicture =   "ABMRubro.frx":030A
      Enabled         =   0   'False
      Height          =   720
      Left            =   4095
      Picture         =   "ABMRubro.frx":0614
      Style           =   1  'Graphical
      TabIndex        =   5
      Top             =   3480
      Width           =   840
   ...
   Begin VB.CommandButton CmdGrabar 
      Caption         =   "&Guardar"
      DisabledPicture =   "ABMRubro.frx":0C28
      Enabled         =   0   'False
      Height          =   720
      Left            =   2385
      Picture         =   "ABMRubro.frx":0F32
      Style           =   1  'Graphical
      TabIndex        =   3
      Top             =   3480
      Width           =   840
...
Private Sub CmdBuscAprox_Click()
    ...
    Screen.MousePointer = 11
    Me.Refresh
    sql = "SELECT RUB_CODIGO,RUB_DESCRI FROM RUBROS "
    sql = sql & " WHERE LNA_CODIGO=" & cboLineas.ItemData(cboLineas.ListIndex)
    If Trim(TxtDescriB) <> "" Then sql = sql & " AND RUB_DESCRI LIKE '%" & Trim(TxtDescriB) & "%'"
...
Private Sub cmdGrabar_Click()
    ...
    If TxtCodigo.Text <> "" Then
        sql = "UPDATE RUBROS "
        sql = sql & " SET RUB_DESCRI = " & XS(TxtDescrip)
        sql = sql & " WHERE RUB_CODIGO = " & XN(TxtCodigo)
        sql = sql & " AND LNA_CODIGO=" & cboLineas.ItemData(cboLineas.ListIndex)
        DBConn.Execute sql
    ...
    Else
        TxtCodigo = "1"
        sql = "SELECT MAX(RUB_CODIGO) as maximo FROM RUBROS"
        rec.Open sql, DBConn, adOpenStatic, adLockOptimistic
        If Not IsNull(rec.Fields!Maximo) Then TxtCodigo = XN(rec.Fields!Maximo) + 1
        rec.Close
        
        sql = "INSERT INTO RUBROS(LNA_CODIGO,RUB_CODIGO,RUB_DESCRI) "
        sql = sql & " VALUES ("
...
Private Sub TxtCodigo_LostFocus()
    If TxtCodigo.Text <> "" Then
        ...
        rec.Open sql, DBConn, adOpenStatic, adLockOptimistic
        If rec.EOF = False Then
         TxtDescrip.Text = rec!RUB_DESCRI
        Else
