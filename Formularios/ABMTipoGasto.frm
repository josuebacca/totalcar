...
Begin VB.Form ABMTipoGasto 
   BorderStyle     =   1  'Fixed Single
   Caption         =   " ABM de Tipo de Gasto"
   ClientHeight    =   3810
   ClientLeft      =   1425
   ClientTop       =   2340
   ClientWidth     =   5895
   ...
   Begin VB.CommandButton CmdSalir 
      Caption         =   "&Salir"
      DisabledPicture =   "ABMTipoGasto.frx":0000
      Height          =   735
      Left            =   4950
      Picture         =   "ABMTipoGasto.frx":030A
      Style           =   1  'Graphical
      TabIndex        =   5
      Top             =   3030
      Width           =   855
   ...
   Begin VB.CommandButton CmdBorrar 
      Caption         =   "&Eliminar"
      DisabledPicture =   "ABMTipoGasto.frx":0614
      Height          =   735
      Left            =   4080
      Picture         =   "ABMTipoGasto.frx":091E
      Style           =   1  'Graphical
      TabIndex        =   4
      Top             =   3030
      Width           =   855
   ...
   Begin VB.CommandButton CmdNuevo 
      Caption         =   "&Nuevo"
      DisabledPicture =   "ABMTipoGasto.frx":0C28
      Height          =   735
      Left            =   3210
      Picture         =   "ABMTipoGasto.frx":0F32
      Style           =   1  'Graphical
      TabIndex        =   3
      Top             =   3030
      Width           =   855
   ...
   Begin VB.CommandButton CmdGrabar 
      Caption         =   "&Aceptar"
      DisabledPicture =   "ABMTipoGasto.frx":123C
      Height          =   735
      Left            =   2340
      Picture         =   "ABMTipoGasto.frx":1546
      Style           =   1  'Graphical
      TabIndex        =   2
      Top             =   3030
      Width           =   855
   ...
   Begin TabDlg.SSTab TabTB 
      ...
      EndProperty
      TabCaption(0)   =   "&Datos"
      TabPicture(0)   =   "ABMTipoGasto.frx":1850
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).Control(0)=   "Frame3"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "&Buscar"
      TabPicture(1)   =   "ABMTipoGasto.frx":186C
      ...
      Begin VB.Frame Frame3 
         Caption         =   " Datos del Tipo de Gasto"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   9.75
            Charset         =   0
   ...
   End
End
Attribute VB_Name = "ABMTipoGasto"
...
Private Sub CmdBorrar_Click()
    ...
    If Trim(TxtCodigo) <> "" Then
        If Trim(TxtCodigo) = 5 Then
            MsgBox "No se puede eliminar el Tipo de Gasto " & Trim(TxtDescrip) & " ", vbInformation
        Else
            ...
            DBConn.Execute "DELETE FROM TIPO_GASTO WHERE TGT_CODIGO = " & XN(TxtCodigo)
...
Private Sub CmdBuscAprox_Click()
    ...
    Me.MousePointer = 11
    Me.Refresh
    sql = "SELECT TGT_CODIGO,TGT_DESCRI FROM TIPO_GASTO "
    If Trim(TxtDescriB) <> "" Then sql = sql & "WHERE TGT_DESCRI LIKE '" & Trim(TxtDescriB) & "%'"
...
Private Sub CmdGrabar_Click()
    ...
    sql = "SELECT TGT_CODIGO FROM TIPO_GASTO WHERE TGT_CODIGO = " & XN(TxtCodigo)
    rec.Open sql, DBConn, adOpenStatic, adLockOptimistic
    If rec.EOF = False Then
        DBConn.Execute "UPDATE TIPO_GASTO SET TGT_DESCRI = " & XS(TxtDescrip) & _
                       " WHERE TGT_CODIGO = " & XN(TxtCodigo)
    Else
        Set Rec2 = New ADODB.Recordset
        TxtCodigo = "1"
        sql = "SELECT MAX(TGT_CODIGO) as maximo FROM TIPO_GASTO"
        Rec2.Open sql, DBConn, adOpenStatic, adLockOptimistic
        If Not IsNull(Rec2.Fields!Maximo) Then TxtCodigo = XN(Rec2.Fields!Maximo) + 1
        Rec2.Close
        DBConn.Execute "INSERT INTO TIPO_GASTO(TGT_CODIGO,TGT_DESCRI) VALUES(" & XN(TxtCodigo) & "," & XS(TxtDescrip) & ")"
    End If
    rec.Close
    Screen.MousePointer = 1
    cmdNuevo_Click
...
Private Sub TxtCodigo_LostFocus()
    If TxtCodigo.Text <> "" Then
        ...
        sql = sql & " WHERE TGT_CODIGO=" & XN(TxtCodigo.Text)
        rec.Open sql, DBConn, adOpenStatic, adLockOptimistic
        If rec.EOF = False Then
            TxtDescrip.Text = rec!TGT_DESCRI
