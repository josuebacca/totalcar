...
Begin VB.Form ABMPresentacion 
   ...
   Begin VB.CommandButton CmdSalir 
      ...
      Height          =   720
      Left            =   4950
      Picture         =   "ABMPresentacion.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   7
      Top             =   3480
      Width           =   840
   ...
   Begin VB.CommandButton CmdBorrar 
      Caption         =   "&Eliminar"
      DisabledPicture =   "ABMPresentacion.frx":030A
      Enabled         =   0   'False
      Height          =   720
      Left            =   4095
      Picture         =   "ABMPresentacion.frx":0614
      Style           =   1  'Graphical
      TabIndex        =   6
      Top             =   3480
      Width           =   840
   ...
   Begin VB.CommandButton CmdNuevo 
      ...
      Height          =   720
      Left            =   3240
      Picture         =   "ABMPresentacion.frx":091E
      Style           =   1  'Graphical
      TabIndex        =   5
      Top             =   3480
      Width           =   840
   ...
   Begin VB.CommandButton CmdGrabar 
      Caption         =   "&Guardar"
      DisabledPicture =   "ABMPresentacion.frx":0C28
      Enabled         =   0   'False
      Height          =   720
      Left            =   2385
      Picture         =   "ABMPresentacion.frx":0F32
      Style           =   1  'Graphical
      TabIndex        =   4
      Top             =   3480
      Width           =   840
   ...
   Begin TabDlg.SSTab TabTB 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.CommandButton CmdBuscAprox 
            ...
            Left            =   4770
            MaskColor       =   &H000000FF&
            Picture         =   "ABMPresentacion.frx":1274
            Style           =   1  'Graphical
            TabIndex        =   9
            ToolTipText     =   "Buscar"
            Top             =   225
   ...
   End
End
Attribute VB_Name = "ABMPresentacion"
...
Private Sub cboLineas_LostFocus()
    If cboLineas.ListIndex <> -1 Then
        cargocboRubro
    End If
End Sub
Private Sub cargocboRubro()
    sql = "SELECT * FROM RUBROS  "
    sql = sql & " WHERE LNA_CODIGO = " & cboLineas.ItemData(cboLineas.ListIndex)
...
Private Sub CmdBorrar_Click()
    ...
    If Trim(TxtCodigo) <> "" Then
        resp = MsgBox("Seguro desea eliminar el Rubro: " & Trim(TxtDescrip) & "? ", 36, "Eliminar:")
        If resp <> 6 Then Exit Sub
        
        Screen.MousePointer = 11
        lblEstado.Caption = "Eliminando ..."
        
        DBConn.Execute "DELETE FROM TIPO_PRESENTACION WHERE TPRE_CODIGO = " & XN(TxtCodigo)
        lblEstado.Caption = ""
...
Private Sub CmdBuscAprox_Click()
    ...
    Screen.MousePointer = 11
    Me.Refresh
    sql = "SELECT TPRE_CODIGO,TPRE_DESCRI FROM TIPO_PRESENTACION "
    sql = sql & " WHERE LNA_CODIGO = " & cboLineas.ItemData(cboLineas.ListIndex) & ""
    If cborubro.ListIndex <> -1 Then sql = sql & " AND RUB_CODIGO = " & cborubro.ItemData(cborubro.ListIndex) & ""
    If Trim(TxtDescriB) <> "" Then sql = sql & "AND TPRE_DESCRI LIKE '" & Trim(TxtDescriB) & "%'"
...
Private Sub CmdGrabar_Click()
    ...
    If TxtCodigo.Text <> "" Then
        sql = "UPDATE TIPO_PRESENTACION "
        sql = sql & " SET TPRE_DESCRI = " & XS(TxtDescrip)
        sql = sql & ", LNA_CODIGO = " & cboLineas.ItemData(cboLineas.ListIndex)
        sql = sql & ", RUB_CODIGO = " & cborubro.ItemData(cborubro.ListIndex)
        sql = sql & " WHERE TPRE_CODIGO = " & XN(TxtCodigo)
        DBConn.Execute sql
        
    Else
        TxtCodigo = "1"
        sql = "SELECT MAX(TPRE_CODIGO) as maximo FROM TIPO_PRESENTACION"
        rec.Open sql, DBConn, adOpenStatic, adLockOptimistic
        If Not IsNull(rec.Fields!Maximo) Then TxtCodigo = XN(rec.Fields!Maximo) + 1
        rec.Close
        
        sql = "INSERT INTO TIPO_PRESENTACION(LNA_CODIGO, RUB_CODIGO,TPRE_CODIGO,TPRE_DESCRI) "
        sql = sql & " VALUES ("
        sql = sql & cboLineas.ItemData(cboLineas.ListIndex) & ","
...
End Sub
Private Sub CargoCboLines()
sql = "SELECT * FROM LINEAS ORDER BY LNA_DESCRI"
    rec.Open sql, DBConn, adOpenStatic, adLockOptimistic
    If rec.EOF = False Then
        rec.MoveFirst
        Do While rec.EOF = False
            cboLineas.AddItem Trim(rec!LNA_DESCRI)
            cboLineas.ItemData(cboLineas.NewIndex) = rec!LNA_CODIGO
            rec.MoveNext
        Loop
...
Private Sub TxtCodigo_LostFocus()
    If TxtCodigo.Text <> "" Then
        ...
        rec.Open sql, DBConn, adOpenStatic, adLockOptimistic
        If rec.EOF = False Then
         TxtDescrip.Text = rec!TPRE_DESCRI
        Else
