...
Begin VB.Form ABMConceptoNotaCredito 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "ABM de Conceptos Nota de Crédito"
   ClientHeight    =   4245
   ClientLeft      =   4065
   ClientTop       =   3105
   ClientWidth     =   5895
   ...
   Begin VB.CommandButton CmdSalir 
      ...
      Height          =   720
      Left            =   4950
      Picture         =   "ABMConceptoNotaCredito.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   5
      Top             =   3480
      Width           =   840
   ...
   Begin VB.CommandButton CmdBorrar 
      Caption         =   "&Eliminar"
      DisabledPicture =   "ABMConceptoNotaCredito.frx":030A
      Enabled         =   0   'False
      Height          =   720
      Left            =   4095
      Picture         =   "ABMConceptoNotaCredito.frx":0614
      Style           =   1  'Graphical
      TabIndex        =   4
      Top             =   3480
      Width           =   840
   ...
   Begin VB.CommandButton CmdNuevo 
      ...
      Height          =   720
      Left            =   3240
      Picture         =   "ABMConceptoNotaCredito.frx":091E
      Style           =   1  'Graphical
      TabIndex        =   3
      Top             =   3480
      Width           =   840
   ...
   Begin VB.CommandButton CmdGrabar 
      Caption         =   "&Guardar"
      DisabledPicture =   "ABMConceptoNotaCredito.frx":0C28
      Enabled         =   0   'False
      Height          =   720
      Left            =   2385
      Picture         =   "ABMConceptoNotaCredito.frx":0F32
      Style           =   1  'Graphical
      TabIndex        =   2
      Top             =   3480
      Width           =   840
   ...
   Begin TabDlg.SSTab TabTB 
      ...
      EndProperty
      TabCaption(0)   =   "&Datos"
      TabPicture(0)   =   "ABMConceptoNotaCredito.frx":123C
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).Control(0)=   "Frame3"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "&Buscar"
      TabPicture(1)   =   "ABMConceptoNotaCredito.frx":1258
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.CommandButton CmdBuscAprox 
            ...
            Left            =   4770
            MaskColor       =   &H000000FF&
            Picture         =   "ABMConceptoNotaCredito.frx":1274
            Style           =   1  'Graphical
            TabIndex        =   7
            ToolTipText     =   "Buscar"
            Top             =   225
   ...
   End
End
Attribute VB_Name = "ABMConceptoNotaCredito"
...
Private Sub CmdBorrar_Click()
    ...
    If Trim(TxtCodigo) <> "" Then
        resp = MsgBox("Seguro desea eliminar el Concepto: " & Trim(TxtDescrip) & "? ", 36, "Eliminar:")
        If resp <> 6 Then Exit Sub
        
        Screen.MousePointer = 11
        lblEstado.Caption = "Eliminando ..."
        
        DBConn.Execute "DELETE FROM CONCEPTO_NOTA_CREDITO WHERE CNC_CODIGO = " & XN(TxtCodigo)
        lblEstado.Caption = ""
...
Private Sub CmdBuscAprox_Click()
    ...
    Screen.MousePointer = 11
    Me.Refresh
    sql = "SELECT CNC_CODIGO,CNC_DESCRI FROM CONCEPTO_NOTA_CREDITO "
    If Trim(TxtDescriB) <> "" Then sql = sql & "WHERE CNC_DESCRI LIKE '%" & Trim(TxtDescriB) & "%'"
    sql = sql & " ORDER BY CNC_DESCRI"
...
Private Sub CmdGrabar_Click()
    ...
    If TxtCodigo.Text <> "" Then
        sql = "UPDATE CONCEPTO_NOTA_CREDITO "
        sql = sql & " SET CNC_DESCRI = " & XS(TxtDescrip)
        sql = sql & " WHERE CNC_CODIGO = " & XN(TxtCodigo)
        DBConn.Execute sql
        
    Else
        TxtCodigo = "1"
        sql = "SELECT MAX(CNC_CODIGO) as maximo FROM CONCEPTO_NOTA_CREDITO"
        rec.Open sql, DBConn, adOpenStatic, adLockOptimistic
        If Not IsNull(rec.Fields!Maximo) Then TxtCodigo = XN(rec.Fields!Maximo) + 1
        rec.Close
        
        sql = "INSERT INTO CONCEPTO_NOTA_CREDITO(CNC_CODIGO,CNC_DESCRI) "
        sql = sql & " VALUES ("
...
Private Sub cmdSalir_Click()
    Unload Me
    Set ABMConceptoNotaCredito = Nothing
End Sub
...
Private Sub TxtCodigo_LostFocus()
    If TxtCodigo.Text <> "" Then
        ...
        rec.Open sql, DBConn, adOpenStatic, adLockOptimistic
        If rec.EOF = False Then
         TxtDescrip.Text = rec!CNC_DESCRI
        Else
